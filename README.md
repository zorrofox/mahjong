...
- 七对（七對）为合法胡牌型，计 +3 番
- 声索窗口 30 秒倒计时，归零自动跳过
- **番数驱动筹码结算**：港式 `unit = min(64, 2^(番-1))`，庄家双倍，荣和放炮全包；大连所有玩家统一公式 `unit = 2^(总番-1)`，输家在赢家番数基础上叠加未开门/三家门清加番后各自结算；杠钱仅胡牌者收取（荒庄不收）；跨局累计，初始 1000 筹码
- **大连番型**：基础(+1)、自摸(+1)/放炮(+1，互斥)、夹胡(+1，坎张，自摸/点炮均可)、庄家(+1)、杠上开花(+2)、抢杠胡(+2，叠加放炮+1)、冲宝(+2，宝牌=等待张)、摸宝(+1，仅自摸)
- **大连输家加番**：未开门（该输家无副露）+1；三家门清（三位输家均无副露）每位各 +1；自摸/点炮均适用
- **大连宝牌**：首位上听触发骰子选宝，宝牌从牌墙后端取出不放回；出牌阶段可选择「看宝/不看宝」（默认不看，出牌时弹窗询问，8秒超时自动看宝）；不看宝仍可正常胡牌
- **胡牌番数详情**：游戏结束时展示本局达成的番型列表及合计番数；结算弹窗同时显示杠钱明细
