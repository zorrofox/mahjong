...
// ── E2E 测试钩子（顶层立即暴露，不依赖 DOMContentLoaded）──────────────────
// 使用闭包访问 let 变量（let 不在 window 上，需通过函数暴露）
window._baoTest = {
  getHideBao:        () => _hideBao,
  getBaoPeekOffered: () => _baoPeekOffered,
  getBaoTile:        () => _baoTile,
  getMyPlayerIdx:    () => myPlayerIdx,
  resetState:        () => {
    _hideBao = true; _baoPeekOffered = false; _baoTile = null;
    if (typeof updateHideBaoBtn === 'function') updateHideBaoBtn();
    if (typeof updateBaoBadge   === 'function') updateBaoBadge(null);
  },
  // 将指定玩家加入内部 _tenpaiPlayers（let 变量，window 上不可见）
  setTenpai: (idx) => {
    if (!_tenpaiPlayers.includes(idx)) _tenpaiPlayers.push(idx);
    if (typeof updateHideBaoBtn === 'function') updateHideBaoBtn();
  },
};
...
if (typeof globalThis !== 'undefined' && typeof window === 'undefined') {
  ...
  globalThis._mahjongTestExports = { getHandTiles, getHandCount, tileToDisplay, formatPhase, autoSelectChow, getAllChows, escapeHtml, TILE_MAP, sortHandTiles, TILE_SVG_MAP, makeTileEl, makeClaimBtn, selectTile, showGameOverModal, playDiscardEffect, playDrawEffect, _resetSharedAC,
    ...
    getBaoPeekOffered:() => _baoPeekOffered,
    getBaoTile:       () => _baoTile,
    resetBaoState:    () => { _hideBao = true; _baoPeekOffered = false; _baoTile = null; updateHideBaoBtn(); updateBaoBadge(null); },
  };
}
