...
class GameState:
    ...
    def _reveal_bao_from_wall(self, dice: int) -> str:
        """
        从牌墙后端取出宝牌并返回其牌种。

        真实规则：「从牌墙后面往前数 dice 摞（每摞2张），翻开该张」
        - 宝牌从牌墙**取出**（不放回），作为野牌标识公开揭示
        - 牌墙中其余同种牌（最多3张）才是玩家实际能摸到的野牌
        - 从后端取出确保不影响正常摸牌顺序

        dice=1 → 取倒数第2张；dice=6 → 取倒数第12张
        使用 max(0, ...) 防止牌墙过短时越界。
        """
        idx = max(0, len(self.wall) - dice * 2)
        return self.wall.pop(idx)
    ...
    def _effective_bao_for_ron(self, player_idx: int, winning_tile: str) -> Optional[str]:
        """
        点炮（荣和）时的宝牌效果：
        - 打出的牌不是宝牌 → 正常返回宝牌（手中已有宝牌可充当野牌）
        - 打出的牌是宝牌  → 返回 None（禁止把别人打出的宝牌当野牌来声索胡；
                            只允许冲宝：即宝牌恰好是结构性等待张，直接荣和）
        """
        bao = self._effective_bao(player_idx)
        return None if winning_tile == bao else bao
    ...
    def check_and_trigger_bao(self) -> Optional[dict]:
        ...
        for i, player in enumerate(self.players):
            ...
            if not self.bao_declared:
                ...
                self.bao_tile = self._reveal_bao_from_wall(dice)
    ...
    def declare_win(self, player_idx: int) -> dict:
        ...
        if self.phase == "discarding" and player_idx == self.current_turn:
            ...
            if self.ruleset == "dalian":
                if not is_winning_hand_dalian(effective_hand, len(player.melds), player.melds,
                                               bao_tile=self._effective_bao_for_ron(player_idx, tile)):
