...
荣和（放炮）：只有放炮者付钱，其余两家不付
  放炮番已在赢家番数中（+1），放炮者实付 = 2^(赢家总番 + 未开门 + 三家门清 - 1)
自摸：三家各按自己所受的番数付钱
  自摸番已在赢家番数中（+1），每位输家实付 = 2^(赢家总番 + 未开门 + 三家门清 - 1)
  未开门（该输家无副露）+1 番（加在该输家身上）
  三家门清（三位输家均无副露）+1 番（每位输家叠加）
荒庄（流局）：无筹码结算；杠钱也不收
杠钱结算：**只有胡牌者的杠**才结算（wiki 原文：「只有胡牌者才算杠分」）；明杠 1×底注/家，暗杠 2×底注/家，三家均付给胡牌者；荒庄不收杠钱
